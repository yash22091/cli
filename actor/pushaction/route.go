...
func (actor Actor) GetGeneratedRoute(manifestApp manifest.Application, host string, orgGUID string, spaceGUID string, knownRoutes []v2action.Route) (v2action.Route, Warnings, error) {
	var (
	...
	if manifestApp.Domain == "" {
		...
		if getDomainsErr != nil {
			...
			return v2action.Route{}, warnings, getDomainsErr
		}
		if len(desiredDomains) == 0 {
			log.Errorln("could not find provided domains '%s':", manifestApp.Domain)
			return v2action.Route{}, warnings, v2action.DomainNotFoundError{Name: manifestApp.Domain}
		}
