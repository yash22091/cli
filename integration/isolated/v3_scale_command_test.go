	...
	Describe("help", func() {
		Context("when --help flag is set", func() {
			It("Displays command usage to output", func() {
				...
				Eventually(session.Out).Should(Say("v3-ssh - Display health and status for an app"))
				Eventually(session.Out).Should(Say("USAGE:"))
				Eventually(session.Out).Should(Say("cf v3-ssh APP_NAME [--guid]"))
	...
	Context("when the environment is set up correctly", func() {
		...
		Context("when the app exists", func() {
			...
			Context("when the --guid flag is given", func() {
				...
				It("displays the app guid", func() {
					session := helpers.CF("v3-ssh", "--guid", appName)
		...
		Context("when the app does not exist", func() {
			It("displays app not found and exits 1", func() {
				...
				session := helpers.CF("v3-ssh", invalidAppName)
				userName, _ := helpers.GetCredentials()
